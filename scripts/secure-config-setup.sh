...
# Function to check if config is in Danger List
is_dangerous(){
    local name=$1
...
discover_config_candidates() {
    for dir in "$CONFIG_DIR"/*/; do
        ...
        # Skip dangerous
        if is_dangerous "$app_name"; then
            color_echo "$RED" "Skipping dangerous config: $app_name"
            continue
        fi

        # Skip already migrated
        if [ -d "$DOTFILES_DIR/$app_name" ]; then
            continue
        fi

        # Skip known cache/state pattern
        case "$app_name" in 
            cache|logs|session|state|Trash)
                continue
                ;;
        esac

        color_echo "$BLUE" "$app_name"
    done
}
...
migrate_home_dotfiles() {
    ...
    for entry in "${HOME_DOTFILES[@]}"; do
        ...
        sensitive_content=$(grep -Ri -E "(token|key|password|secret|api|auth|session|credential|private)" "$dest_file" 2>/dev/null | head -3 || true)
        
        if [ -n "$sensitive_content" ]; then
            color_echo "$YELLOW" "POTENTIAL SENSITIVE CONTENT found in $dotfile:"
            color_echo "$YELLOW" "$sensitive_content"
...
check_and_migrate_config() {
    ...
    if [ -d "$dest_package" ]; then
        color_echo "$YELLOW" "$app_name already migrated, skipping..."
        return
    fi
    ...
    if [ -n "$sensitive_files" ]; then
        ...
        color_echo "$YELLOW" "$sensitive_files"
...
main() {
    ...
    color_echo "$CYAN" "Discovering config candidates..."
    while read -r app; do
        check_and_migrate_config "$app"
    done < <(discover_config_candidates)
    ...
    color_echo "$CYAN" "2. Test with: stow -n package-name (dry run)"
    color_echo "$CYAN" "3. Add to git: git add package-name/"
    color_echo "$CYAN" "4. Commit: git commit -m 'Add package-name config'"
}

# Safety check
if [ "$PWD" != "$DOTFILES_DIR" ]; then
    cd "$DOTFILES_DIR" || {
        color_echo "$RED" "Failed to enter $DOTFILES_DIR"
        exit 1
    }
fi
